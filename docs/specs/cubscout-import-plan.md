...
**Status:** Plan, not yet scoped with engineering (extracted for review)
**Date:** 2026-05-02
...
2. **GitHub issue #2985 closed on the higher-level constructs question
   ...
   in v0.1.36 reflects this. The canonical work for the ConfighubOps
   dogfood is done.

3. **Pattern 1 (OCI + repoint) is demonstrated working.** Argo CD was set
   up as a Component in the Kubara org on 2026-05-01: argocd-base
   plus argocd-kubara deployment Variant, ingress and OIDC, working
   end-to-end. An urgent image-pin change was applied on 2026-05-02
...
Argo CVE pattern demonstrated on 2026-05-02 then becomes
...
2. A new flow — call it `cub-scout onboard` for now, naming TBD — that
   performs Pattern 1 takeover: scan, propose, register,
...
6. A worked example each for Argo and Flux, with verifiable proof
   chains (Kubara as one of them, given the existing setup).
...
(Tracks 1–5 as detailed in the source plan. Owners: TBD.)
...
1. **Byte-equivalence decision.** Resolves Q-BYTEEQ-1
...
2. **Final command name.** Resolves Q-NAMING-1. The spec uses
...
- **Track 1 — vocabulary alignment.** Can start the moment Q-BASE-1's
  shape is confirmed. The renames are mechanical and the spec
...
- **Adaptation (clone-then-replace-placeholders flow).** This has been
  flagged as the next product gap and is under discussion. It is a
  sibling problem to import, not a subset of it. Lives on a separate
  track. The cub-scout
...
| Question | Owner | Spec |
|----------|-------|------|
| Q-URL-1: Component / Variant / Base Variant URL paths in Promotions UI | TBD | vocabulary |
| Q-LABELS-1: per-Variant Space labels vs a parent Component Space | TBD | vocabulary |
| Q-CONN-1: does Connection v1 spec land first | TBD | vocabulary |
| Q-MIGRATION-1: deprecation runway length (2 vs 3 minor releases) | TBD | vocabulary |
| Q-BASE-1: emit a Base Variant for every Component | TBD | vocabulary |
| Q-BYTEEQ-1: ConfigHub OCI render byte-equivalence with controller | TBD | takeover |
| Q-BYTEEQ-2: accept-and-reconcile escape hatch vs ship-gate | TBD | takeover |
| Q-NAMING-1: final command name (`onboard` / `attach` / `adopt`) | TBD | takeover |
| Q-CUB-CMD-1: which `cub` subcommand performs the source repoint | ConfigHub side | takeover |
| Q-ROLLBACK-1: `--rollback` flag vs separate `offboard` command | TBD | takeover |
| Q-APPSET-1: ApplicationSet warn-only vs refuse for v1 | TBD | takeover |
| Q-OCI-AUTH-1: OCI pull auth provisioning at takeover | TBD | takeover |
| Q-AUDIT-1: `onboard-proof.json` as new format vs audit-log extension | TBD | takeover |
...
The byte-equivalence conversation between the cub-scout and ConfigHub sides. Everything
