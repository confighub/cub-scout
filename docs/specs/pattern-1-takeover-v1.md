...
**Owner (proposed):** TBD (cub-scout-side flow, plus the OCI
byte-equivalence question on the ConfigHub side)
...
(Kubara org) with a follow-on image-pin change on 2026-05-02
...
Final name to be set — see open questions below. The plan uses
...
This is the four-layer proof captured manually for the
...
- **Q-BYTEEQ-1 (the hard one).** Does ConfigHub's OCI render produce
  ...
  that set stable across ConfigHub releases? Owner: TBD.
- **Q-BYTEEQ-2.** If byte-equivalence is not achievable, do we need a
  ...
  byte-equivalence as a v1 ship gate? Owner: TBD.
- **Q-NAMING-1.** Final command name. `onboard` is the working name.
  Alternatives: `attach`, `adopt`, `import --takeover`. Owner: TBD.
...
- **Q-APPSET-1.** ApplicationSet support strategy for v1: do we
  ...
  for v1; confirm on the ConfigHub side.
