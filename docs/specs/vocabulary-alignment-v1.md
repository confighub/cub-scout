...
**Owner (proposed):** TBD (cub-scout import path)
...
- **Q-URL-1.** What are the exact path strings for Component, Variant,
  and Base Variant detail pages in the Promotions UI? Owner: TBD
...
- **Q-LABELS-1.** When a Component has many Variants, do the labels
  ...
  question is whether to also emit a `Component` Space. Owner: TBD.
- **Q-CONN-1.** Does the Connection v1 spec land before this work
  ...
  typing pass is added later. Owner: TBD (drives the spec ask).
...
- **Q-BASE-1.** Today the import path infers a single `App` per
  ...
  As of 2026-05-02, the system distinguishes Base from
  ...
  promote against it. Confirm shape on the ConfigHub side; the underlying model
  is settled.
  ...
  not yet a first-class flow. Out of v1 onboarding scope. The
  adaptation question is owned on a separate track; this plan
  does not absorb it.
