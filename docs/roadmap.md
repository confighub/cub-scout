...
- [x] **v2 real-time emission** — `watch --emit-receipt-on <event-types>` with the event-type set `drift.detected` / `ownership.changed` actually building receipts in v1; other types (`resource.discovered` / `scan.finding`) accept the flag for forward-compat but pass through silently (a startup warning lists them). Receipt-build failures non-fatal; rate-limited stderr warnings (first 10 + summary every 100) — shipped in #463 (#449 v1)
- [ ] v2 aggregate-with-discovery half — `cub-scout receipt verify --scope namespace/<ns>` auto-discovery + `synthetic-aggregate://` subject construction + max-severity verdict synthesis — tracked in #448 (issue stays open for this scope)
- [ ] v2 backpressure / batching — high-frequency event handling for `--emit-receipt-on`, plus the `resource.discovered` / `scan.finding` support gated on the backpressure design — tracked in #449 (issue stays open for this scope)
...
**`#446` v1 + v2 status: shipped on `main` (#454 + #455 + #456 + #463).** The capability is documented in `docs/reference/json-contracts.md` § Receipt Contract, `docs/reference/commands.md` § receipt, and `docs/reference/cli-contract.md` § `cub-scout receipt`, with canonical examples under `examples/receipts/` (including the CI-gate walkthrough at `examples/receipts/ci-gate/`) and a full test suite. Parent issue #446 closed; #451 closed; #448 / #449 remain open for the v2 deferrals above.
...
Spin-off issues status: **#451 (`--fail-on` exit semantics) closed via #463.** #448 (aggregate / chained receipts) and #449 (`watch --emit-receipt-on`) both have v1 / chained-half scope shipped via #463; the issues stay open for the deferred halves (aggregate-with-discovery for #448; backpressure for #449). #450 (source-truth help-text drift) closed earlier. PolicyReport / Kyverno integration remains an external adapter project, not cub-scout's job.
