...
Shows Git repository structure from Flux GitRepositories, ArgoCD Applications, and ArgoCD ApplicationSets (including generated-application relationships when discoverable).
...
ArgoCD ApplicationSets (1)
────────────────────────────────────────────────────────────────────
  └── argocd/workloads-generator (argo/applicationset/{{env}})
      generators: git,list
      -> workloads-dev
      -> workloads-prod

════════════════════════════════════════════════════════════════════
Summary: 2 GitRepositories │ 1 ArgoCD ApplicationSet │ 1 ArgoCD Application │ 40 resources
```

### App-of-Apps vs ApplicationSet (quick comparison)

- App-of-Apps: child Application will show `child of application/<parent-app>`.
- ApplicationSet: generated Application will show `generated by applicationset/<name>`, and the ApplicationSet section lists generated children.
- Missing ApplicationSet link:
  - explicit ownerReference -> `orphan applicationset/<name> (missing from cluster)`
  - inferred label/annotation -> `unresolved applicationset/<name> (inferred link only)`

Example relationship hints in `tree git` output:

```
payments-dev (argo/app-of-apps/apps/payments-dev) [child of application/platform-root]
workloads-dev (argo/applicationset/dev) [generated by applicationset/workloads-generator]
```
