...
| `receipt verify` | Build a typed, fingerprinted receipt asserting a predicate (applied-matches-spec / source-truth-pass / no-manual-edits-since / object-set-matches). `--file <path>` verifies rendered install object sets; `--fail-on <verdict>` for CI gating (exit 2); `--input-attestation <path>` for chained receipts; `--save` to persist to the immutable local store | [Command Reference](commands.md#receipt-verify) | [Receipts](../../examples/receipts/) |
