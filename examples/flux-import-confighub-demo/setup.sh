    ...
    cat <<JSON
{
  ...
  "writes": [
    "kind cluster kind-flux-import-demo",
...
- a kind cluster named flux-import-demo
- current kubeconfig context via kind
...
- cluster evidence via kubectl and flux against kind-flux-import-demo
- ConfigHub evidence via cub when --with-worker is used
- cub-scout evidence via gitops status and tree ownership
