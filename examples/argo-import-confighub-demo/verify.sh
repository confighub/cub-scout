...
KUBE_CONTEXT="kind-argo-import-demo"
SPACE="argo-import-demo"
RENDERER_TOKEN="argocdrenderer"
...
kubectl --context "$KUBE_CONTEXT" get namespace argocd >/dev/null 2>&1 || fail "argocd namespace is missing"

echo "==> Checking reference and brownfield applications"
for app in helm-guestbook kustomize-guestbook myapp-dev myapp-staging myapp-prod; do
    kubectl --context "$KUBE_CONTEXT" get application "$app" -n argocd >/dev/null 2>&1 || fail "ArgoCD Application '$app' is missing"
done
...
echo "==> Checking cub-scout map list"
map_output="$("$CUB_SCOUT" map list 2>&1 || true)"
if [[ -z "${map_output//[[:space:]]/}" ]]; then
    fail "cub-scout map list produced no output"
fi
printf '%s\n' "$map_output"
