...
- **v2.8.0 ConfigHub delivery evidence release**
  ...
  - `map activity --with-confighub` projects the same bounded ConfigHub evidence into timeline rows for live-status writeback, release publication, unit events, event-consumer health, and omissions, and attaches additive live-status evidence to matching Argo Application activity rows when a non-wildcard ConfigHub space and Application name match exactly
...
- **Live delivery observability release slice — merged for v2.7.0** (`#500`)
  ...
  - Safe follow-ups remain tracked in `docs/roadmap.md`: aggregate failures as top-level `doctor` findings, audited actions as history/receipt evidence, deeper controller-family parity omissions, and object-level release/live-status correlation
...
- **MCP gateway** — `mcp serve` exposes a closed, read-only-by-construction tool catalog:
  ...
  - 11 connected tools: `compare_three_way`, `compare_source_truth`, `confighub_changesets`, `confighub_k8s_resources`, `confighub_k8s_types`, `confighub_live_status`, `confighub_releases`, `confighub_resources`, `confighub_unit_events`, `confighub_units`, `confighub_unit_get`
