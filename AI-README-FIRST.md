...
- **v2.8.0 ConfigHub delivery evidence release**
  ...
  - ConfigHub release and unit-event reads are scoped by `--confighub-space`, else `CUB_SPACE`, else the cub context's default space if it has one (reported as `scope.spaceSource`); they are skipped, never widened, when no space is given. They support explicit `--confighub-space '*'`, and are bounded by `--confighub-since`
