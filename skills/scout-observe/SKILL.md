...
phase: verify
allowed-tools: Bash(./cub-scout doctor) Bash(./cub-scout doctor *) Bash(cub-scout doctor) Bash(cub-scout doctor *) Bash(cub scout doctor) Bash(cub scout doctor *) Bash(./cub-scout map list *) Bash(./cub-scout map workloads *) Bash(./cub-scout map orphans *) Bash(./cub-scout map activity *) Bash(./cub-scout map hooks *) Bash(./cub-scout map meaning *) Bash(cub-scout map list *) Bash(cub-scout map workloads *) Bash(cub-scout map orphans *) Bash(cub-scout map activity *) Bash(cub-scout map hooks *) Bash(cub-scout map meaning *) Bash(cub scout map list *) Bash(cub scout map workloads *) Bash(cub scout map orphans *) Bash(cub scout map activity *) Bash(cub scout map hooks *) Bash(cub scout map meaning *) Bash(./cub-scout trace *) Bash(cub-scout trace *) Bash(cub scout trace *) Bash(./cub-scout tree *) Bash(cub-scout tree *) Bash(cub scout tree *) Bash(./cub-scout scan *) Bash(cub-scout scan *) Bash(cub scout scan *) Bash(./cub-scout graph *) Bash(cub-scout graph *) Bash(cub scout graph *) Bash(./cub-scout snapshot *) Bash(cub-scout snapshot *) Bash(cub scout snapshot *) Bash(./cub-scout status) Bash(./cub-scout status *) Bash(cub-scout status) Bash(cub-scout status *) Bash(cub scout status) Bash(cub scout status *) Bash(kubectl get *) Bash(kubectl describe *) Bash(kubectl logs *) Bash(kubectl version) Bash(kubectl config current-context) Bash(kubectl config get-contexts *)
---
...
- **Allowed (read-only):** the specific Observe verbs — `cub-scout doctor`, `cub-scout map list / workloads / orphans / activity / hooks / meaning`, `cub-scout trace`, `cub-scout tree`, `cub-scout scan`, `cub-scout graph`, `cub-scout snapshot`, `cub-scout status` (in all three invocation forms: `cub-scout`, `./cub-scout`, `cub scout`). Plus `kubectl get/describe/logs`, `kubectl version`, `kubectl config current-context / get-contexts`. `cub-scout watch` and `cub-scout bot` are Observe verbs too, but are not auto-approved: its `--webhook` flag sends events to any URL, so the host asks each time; `kubectl config view` is left out because `--raw` prints credentials. **Never** the broad `cub-scout *` wildcard — that would grant `demo` (kubectl-applies), `import apply` (writes to ConfigHub), and `compare --suggest --apply` (writes to ConfigHub).
...
| "Dump the cluster as GSF JSON" | `cub-scout snapshot` | Stable snapshot for AI / agent / audit; includes top-level `observation` metadata |
| "Stream observation events" | `cub-scout watch -n <ns> [--webhook url]` | Webhook / file sinks, namespace / owner / severity filters; events include `observation` metadata |
| "Run an in-cluster observer bot" | `cub-scout bot --webhook <url>` | Same event shape as `watch`, with env-var configuration for Kubernetes manifests and per-event `observation` metadata |
| "What mode am I in? Connection / cluster info" | `cub-scout status` | Standalone vs connected, context, cluster details |

Snapshot-style verbs that support `--format json` emit deterministic JSON for
agents. `map list --format json`, `snapshot`, `watch`, and `bot` also expose
point-in-time `observation.source/mode/observedAt/freshness` metadata so agents
can avoid confusing a stale artifact with a fresh read.
...
- All output is point-in-time. `map list --format json`, `snapshot`, `watch`,
  and `bot` declare this with `observation` metadata. For a durable,
  fingerprinted record of "what cub-scout observed at time T", use the Verify
  verb group (`cub-scout receipt verify`) — see
  [`scout-verify`](../scout-verify/SKILL.md).
