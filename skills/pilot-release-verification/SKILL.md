...
- **Allowed (cub-scout):** Compare verbs (read-only), `history`, `doctor`, `explain`, `trace`, `receipt verify` / `show` / `validate` / `list`, `watch` for the optional event-driven verification track (not auto-approved: its `--webhook` flag sends events to any URL, so the host asks each time). All read-only.
- **Allowed (cluster):** `kubectl get/describe`. NOT `kubectl apply / edit / patch / delete / rollout undo`.
- **Allowed (ConfigHub):** `cub <entity> get / list` for the entities named in allowed-tools. NOT mutating `cub` verbs.
