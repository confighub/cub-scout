...
- **Cross-controller composition**: kro + Crossplane / kro + Flux is possible. Ownership detection precedence is Flux → Argo → Helm → Terraform → ConfigHub → Crossplane → kro → Custom → K8s native. kro runs LATE in the chain, so if Flux labels are also present, the resource classifies as `OwnerFlux` and kro is invisible to the ownership detector.
