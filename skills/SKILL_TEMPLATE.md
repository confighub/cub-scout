...
allowed-tools: <Read set only. Always Bash(./cub-scout *), Bash(cub-scout *), Bash(cub scout *) for the local repo. Add Bash(kubectl get *), Bash(kubectl describe *), Bash(kubectl logs *) where the skill needs cluster reads. Add Bash(cub * get) / Bash(cub * list) / Bash(cub link list *) for connected-mode ConfigHub reads. Add Bash(argocd app get *), Bash(flux get *) for controller diagnostics. NEVER grant Bash(kubectl apply/edit/patch/delete *), Bash(cub * delete), Bash(cub * update), Bash(cub * create), Bash(argocd app sync), or any mutating pattern.>
---
...
- **Read-only-triad invariant:** every skill's `allowed-tools` line must stay inside #410/#428 — see [Tool boundary] above. A skill that adds a mutating pattern is a bug, not a feature.
