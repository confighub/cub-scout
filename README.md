...
| Can I answer broad or repeated questions without hammering live APIs? | `snapshot`, `watch`, `bot`, `summary`, `receipt verify --with-confighub`, `map list --format json`, `map activity --with-confighub --confighub-space <space>`, `doctor --with-confighub --confighub-space <space>`, `gitops status --with-confighub --confighub-space <space> --confighub-since <window>` | Existing captured state, low-cardinality watch/bot events, connected summaries, bounded current-space/time-window ConfigHub reads, `observation.source/mode/observedAt/freshness` metadata on live inventory/event snapshots and stored summary records, and immutable receipts that let repeated reviewers inspect the same frozen evidence instead of re-querying live APIs. |
...
- **Live delivery observability follow-ups** — aggregate controller-resource failures with generated-artifact lineage as top-level `doctor` findings, audited action events as history/receipt evidence, receipt-backed source-freshness ergonomics, deeper object-level correlation for release/event/live-status evidence ([#502](https://github.com/confighub/cub-scout/issues/502)), and parity omissions for controllers without status/source/event/generation evidence.
