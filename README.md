...
| Is delegated delivery healthy? | `doctor --with-confighub`, `gitops status`, MCP `gitops_status`, `map deployers`, `map activity --with-confighub`, `trace` | Controller-reported backend, transport, source/build/apply/sync stages, first-class aggregate delivery resources, scope-level delivery rollups, ConfigHub delivery timeline rows, reason/message, and explicit evidence gaps when status is incomplete. |
...
| Can I move to the next task, wait, or retry delivery? | `receipt verify`, `compare three-way`, `doctor --with-confighub`, `map activity --with-confighub` | A read-only decision frame that separates "not applied yet", "still converging", "runtime failure", stale/failed delivery feedback, recent delivery events, and missing evidence. |
...
| Can I answer broad or repeated questions without hammering live APIs? | `snapshot`, `watch`, `bot`, `summary`, receipts, `map activity --with-confighub --confighub-space <space>`, `doctor --with-confighub --confighub-space <space>`, `gitops status --with-confighub --confighub-space <space> --confighub-since <window>` | Existing captured state, low-cardinality watch/bot events, connected summaries, bounded current-space/time-window ConfigHub reads, observed timestamps/fingerprints where present, and immutable receipts for repeated review, fleet triage, and audit paths. |
...
- **Delivery evidence** — `gitops status --with-confighub`, `doctor --with-confighub`, and `map activity --with-confighub` add bounded release history, unit events, live-status writeback, event-consumer health, rollups, timeline rows, and omissions for a known space/time window.
