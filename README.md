...
| [v2.11.0 (prepared)](docs/releases/v2.11.0.md)
...
| Is the image actually running the one I intended? | [`release check --check-running-image`](docs/proposals/running-image-identity.md); MCP `release_check` with `check_running_image: true`; `release check --interactive --check-running-image` | **Unreleased in v2.11.0, opt-in and off by default:** compares the digest running in live pods (`.status.containerStatuses[].imageID`) with the intended workload image, via one bounded, selector-scoped pod read per workload. A mutable tag stays `UNKNOWN` (pin to a digest to confirm); a differing digest is `MISMATCH` carrying a multi-architecture caveat, never a false alarm; the headline never claims running when it is unconfirmed. The configuration-bundle digest and the container-image digest are separate identities and are never compared. Not application success. |
...
**v2.11.0 is prepared but not yet published.** It adds identity-linked release
checks (controller-revision evidence, exact OCI release checks, and opt-in
...
[release notes](docs/releases/v2.11.0.md). The install commands above stay on
v2.10.1 until v2.11.0 is tagged.
