...
**Standalone or connected — a mode over all five surfaces.** By default cub-scout
runs **standalone** against one kube context, with no ConfigHub dependency. Add
`--with-confighub` (or use the `confighub_*` MCP tools) to run **connected to
[ConfigHub](https://confighub.com)** for governed desired-vs-live comparison,
delivery history and live-status, fleet and Resource queries, and import
proposals. Connected mode is read-only and additive: it never writes to the
cluster, and standalone stays fully functional without ConfigHub.
...
| Which OCI source or release did connected evidence actually observe? | `trace --with-confighub`, `explain --with-confighub`, single-resource `receipt verify --with-confighub` | **UNRELEASED source branch:** OCI sources recognize the configured registry's `/space/<slug>` form and the legacy target form. An exact release join requires a fresh source/status binding, agreement with the active ConfigHub registry advertised through `/api/info`, and the reported source revision in `Release.ManifestDigest`. A desired URL pin is not observed source evidence, `Release.Digest` is bundle content, and a target-only row is scope context rather than release execution. |
...
| Does this selected controller report the immutable revision I asked for? | `explain Application/api -n delivery --bounded --api-version argoproj.io/v1alpha1 --kube-context <context> --expected-revision <commit-or-digest> --format json`; MCP `explain` with `expected_revision`; TUI `Ctrl+e`, select, then `e` | **v2.11.0:** `controllerRevision` compares an explicit full Git commit or SHA-256 artifact digest with a single-source Application or v1 Kustomization report: `match`, `mismatch`, or `unknown`, with original reported revision and separate controller status. Missing, ambiguous, deleted or unobserved evidence remains unknown. No ConfigHub auth; same two-request cold read and zero-request session reuse, including when changing the expected revision. This is not workload convergence, controller liveness or application success. [Cases, limits and commands](examples/controller-revision/). |
...
| Can a script, CI job or agent check this without a TUI? | [`release check --check-running-image --format json --out report.json --fail-on any-non-pass`](docs/howto/is-this-image-deployed.md#scripts-and-ci); same flags after `cub scout` | No TTY or interactive prompts required. Normal exit 0 means PASS with this gate; exit 2 preserves the non-passing report; setup/output errors are nonzero. **UNRELEASED:** stdout write failures cannot return success. CLI/plugin fixtures verify complete JSON, saved reports and bounded reads: 11 Argo / 15 Flux Kubernetes requests for one Deployment with two Pods. This is a request budget, not a latency SLA. |
| Can I check one release without a cluster-wide scan, and keep the evidence? | `release check --max-objects 100 --out release-check.json`; `--oci-layout <dir>` for a local bundle; TUI `r` refresh | At most 100 desired objects; exact GETs and no inventory LIST. The optional `--check-running-image` adds a bounded structured-selector Pod LIST per eligible workload, plus bounded exact owner reads for the **UNRELEASED** Deployment proof. Actual request counts, context, UID and omissions are recorded. The report embeds existing fingerprinted configuration/workload receipts; the whole report is not signed or an immutable receipt. Watch/bot do not schedule this check yet. |
| Can I reproduce the image check against real controllers, not just fixtures? | [Live acceptance and release gates](docs/releases/image-verification-readiness.md); `bash test/e2e/image-delivery-live.sh` | **UNRELEASED validation tooling:** a disposable kind cluster exercises actual Argo and Flux OCI delivery, complete Deployment execution, capped evidence and configuration drift. The setup creates test resources; Scout stays read-only. Reports preserve dates, request counts and tested binary identity. Local-registry proof is not authenticated ConfigHub publication or application success. |
| Can a stalled credential helper hang my unattended image check? | [Credential and timeout boundaries](docs/howto/is-this-image-deployed.md#run-the-check) | **UNRELEASED bounded reader:** request cancellation terminates exec-auth helpers, credential output is capped and helper stderr is suppressed. Linux/macOS also terminate the helper process group; Windows descendant cleanup still needs the enclosing job timeout. Keep non-interactive credentials and an outer timeout. This is not a timing SLA for every Scout command. |
| Can a large API error bypass the response-size limit? | `release check`; shared bounded reads | **UNRELEASED:** HTTP error bodies are capped before client-go buffers them, including discovery and Pod-list failures. Oversized errors remain failed observations, never PASS. |
| Is the image actually running the one I intended? | [`release check --check-running-image`](docs/howto/is-this-image-deployed.md); MCP `release_check` with `check_running_image: true`; `release check --interactive --check-running-image` | **v2.12.0 shipped:** compares pod-reported image IDs with intended digests, but does not prove every replica's ownership, running state, or status. **UNRELEASED source branch:** Deployment-only full proof checks the exact Pod set, Pod -> ReplicaSet -> Deployment UID chain, current template, generation, positive/equal replica counts, `Running` + `PodReady`, and every regular container's running/ready exact digest. Mutable tags and unresolved index/platform forms stay `unknown`. Not application success. |
| Do I have the exact release identity? | `release check --bundle oci://...@sha256:...` | The bundle digest identifies the literal configuration artifact. The intended container-image digest and pod-reported image digest are separate identities; a desired URL pin is not an observed source revision. |
| Are all replicas running the intended image? | `release check --check-running-image --format json` | **v2.12.0 shipped:** not proved; label-selected image IDs can hide per-pod gaps. **UNRELEASED source branch:** only a Deployment can receive complete ownership/replica proof. StatefulSet, DaemonSet, and Job ownership remains unknown. |
| What does incomplete proof mean? | Inspect `runningImage.workloads[].reason`, `deployment.complete`, and `pods[]`, plus the overall verdict | **UNRELEASED source branch:** missing, malformed, ambiguous, capped, RBAC, stale-generation, race, terminating, old/foreign, duplicate, and zero-replica evidence remains `unknown` / `INCONCLUSIVE`; it never becomes a positive claim. A configuration `BLOCK` still blocks the overall check even when images match. |
...
**UNRELEASED source-branch behavior**. The `v2.12.0` binary retains the
documented label-selected, per-pod completeness gaps; do not read this source
branch documentation as a claim about the latest release.
