...
| Can I answer broad or repeated questions without hammering live APIs? | `snapshot`, `watch`, `bot`, `summary`, `receipt verify --with-confighub`, `map activity --with-confighub --confighub-space <space>`, `doctor --with-confighub --confighub-space <space>`, `gitops status --with-confighub --confighub-space <space> --confighub-since <window>` | Existing captured state, low-cardinality watch/bot events, connected summaries, bounded current-space/time-window ConfigHub reads, observed timestamps/fingerprints where present, and immutable receipts that let repeated reviewers inspect the same frozen evidence instead of re-querying live APIs. |
...
| Can I keep auditable evidence of the check? | `receipt verify`, `receipt verify --with-confighub`, `receipt validate`, `watch --emit-receipt-on`, `bot --emit-receipt-on` | Typed, fingerprinted, immutable evidence receipts for gates, incident closeout, audits, chained checks, live delivery-status snapshots, and real-time watch/bot events. |
