...
| What does incomplete proof mean? | Inspect `runningImage.workloads[].reason`, `deployment.complete`, and `pods[]` | **UNRELEASED source branch:** missing, ambiguous, capped, RBAC, stale-generation, race, terminating, old/foreign, duplicate, and zero-replica evidence remains `unknown` / `INCONCLUSIVE`; it never becomes a positive claim. |
