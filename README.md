...
| Can I check one release without a cluster-wide scan, and keep the evidence? | `release check --max-objects 100 --out release-check.json`; `--oci-layout <dir>` for a local bundle; TUI `r` refresh | At most 100 desired objects; exact GETs, no inventory LIST or pod fan-out. Configuration and convergence share each dated read. Actual request counts, context, UID and omissions are recorded. The report embeds existing fingerprinted configuration/workload receipts; the whole report is not signed or an immutable receipt. Watch/bot do not schedule this check yet. |
