...
- **#409** — source-truth v0.2 cross-surface revision equality. Design pre-baked in the issue. Strategy-scoping context: all `Chain[0].Revision` and `Chain[0].URL` fields are already populated for all strategies (OCI digest, Git SHA, semver, Helm chart version). The normalisation problem is that these identifiers are heterogeneous and incomparable without strategy-aware rules. `confighubai/confighub#4356` interacts with `confighub-oci-argo` equality. Recommended: implement after confirming whether ConfigHub exposes a rendered digest per unit revision (the issue flags this as a cross-repo dependency to verify first).
- **Triad violation: `remedy.go`** — `cmd/cub-scout/remedy.go` contains a `remedy` command that claims cub-scout "executes remediation" and "fixes issues". Genuine architectural triad violation. Needs to be deleted or reframed as evidence-only. A task chip is filed.
