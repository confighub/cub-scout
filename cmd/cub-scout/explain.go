...
func init() {
	...
	explainCmd.Flags().StringVar(&explainConfigHubStaleAfter, "confighub-stale-after", "15m", "Treat ConfigHub live-status observations older than this as stale")
}
...
func buildExplainSummary(result *agent.TraceResult) ExplainSummary {
	...
	summary.DeliveryEvidence = result.DeliveryEvidence
...
func renderExplainText(summary ExplainSummary, mode PresentationMode, explicitMode bool, hintCtx HintContext) string {
	...
	if summary.DeliveryEvidence != nil {
		fmt.Fprintf(&b, "  %s %s\n", label("Delivery evidence"), formatTraceDeliveryEvidenceLine(summary.DeliveryEvidence))
	}
...
func renderExplainMarkdown(summary ExplainSummary, mode PresentationMode, explicitMode bool, hintCtx HintContext) string {
	...
	if summary.DeliveryEvidence != nil {
		fmt.Fprintf(&b, "- **Delivery evidence:** `%s`\n", formatTraceDeliveryEvidenceLine(summary.DeliveryEvidence))
	}
