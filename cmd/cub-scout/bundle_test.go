...
// Bundle Summarize tests

func TestBundleSummarize_JSON(t *testing.T) {
	// Create a bundle with drift findings
	tmpDir := t.TempDir()
	bundleDir := filepath.Join(tmpDir, "summarize-json-bundle")

	writer := agent.NewBundleWriter("v0.19.5-test")
	bundle := &agent.DebugBundle{
		Metadata: agent.BundleMetadata{
			Target: agent.BundleTarget{
				Kind:      "Deployment",
	...
	if summary.FormatVersion != "v1" {
		t.Errorf("FormatVersion = %s, want v1", summary.FormatVersion)
	}
	if summary.Cluster != "cluster-1" {
		t.Errorf("Cluster = %s, want cluster-1", summary.Cluster)
	}
	if summary.Namespace != "prod" {
		t.Errorf("Namespace = %s, want prod", summary.Namespace)
	}
	if summary.Target != "Deployment/api" {
		t.Errorf("Target = %s, want Deployment/api", summary.Target)
	}
	if summary.Changes.DriftCount != 1 {
		t.Errorf("DriftCount = %d, want 1", summary.Changes.DriftCount)
	}
	if summary.Changes.EventCount != 1 {
		t.Errorf("EventCount = %d, want 1", summary.Changes.EventCount)
	}
	if summary.GitContext == nil {
		t.Fatal("GitContext should not be nil")
	}
	if summary.GitContext.Commit != "abc1234567890" {
		t.Errorf("GitContext.Commit = %s, want abc1234567890", summary.GitContext.Commit)
	}
...
func TestBundleSummarize_TicketFormat(t *testing.T) {
	summary := BundleSummary{
		...
		GitContext: &BundleSummaryGitContext{
			Commit: "abc1234567890",
			Branch: "main",
		},
		Changes: BundleSummaryChanges{
			DriftCount: 2,
			EventCount: 5,
			Categories: []string{"capacity", "image"},
			Objects:    []string{"Deployment:payments/api", "Deployment:payments/worker"},
		},
		RiskSignals: []BundleSummaryRiskSignal{
			{Level: "critical", Message: "1 critical drift finding(s)"},
		},
	...
	expectedStrings := []string{
		...
		"[critical]",
		"### Evidence",
		"./debug-bundle-2026-02-06",
		"*Generated by cub-scout*",
	}

	for _, s := range expectedStrings {
		if !strings.Contains(output, s) {
			t.Errorf("Ticket output missing %q", s)
		}
	}
}

func TestBundleSummarize_PRFormat(t *testing.T) {
	summary := BundleSummary{
		FormatVersion: "v1",
		Target:        "Deployment/api",
		CapturedAt:    "2026-02-06T10:30:00Z",
		Changes: BundleSummaryChanges{
			DriftCount: 1,
			Objects:    []string{"Deployment:prod/api"},
		},
		RiskSignals: []BundleSummaryRiskSignal{
			{Level: "warning", Message: "1 warning drift finding(s)"},
		},
		Evidence: BundleSummaryEvidence{
			BundlePath: "./bundle",
		},
	...
	for _, s := range expectedStrings {
		if !strings.Contains(output, s) {
			t.Errorf("PR output missing %q", s)
		}
	}
}

func TestBundleSummarize_SlackFormat(t *testing.T) {
	summary := BundleSummary{
		FormatVersion: "v1",
		Cluster:       "prod",
		Namespace:     "api",
		...
		RiskSignals: []BundleSummaryRiskSignal{
			{Level: "critical", Message: "Critical issue detected"},
		},
		Evidence: BundleSummaryEvidence{
			BundlePath: "./bundle",
		},
	...
	if err := json.Unmarshal([]byte(output), &parsed); err != nil {
		t.Fatalf("Slack output is not valid JSON: %v", err)
	}

	// Verify blocks structure
	blocks, ok := parsed["blocks"].([]interface{})
	if !ok {
		t.Fatal("Slack output missing blocks array")
	}
...
func TestBundleSummarize_ASCIIFormat(t *testing.T) {
	summary := BundleSummary{
		FormatVersion: "v1",
		Cluster:       "prod",
		Namespace:     "api",
	...
	for _, s := range expectedStrings {
		if !strings.Contains(output, s) {
			t.Errorf("ASCII output missing %q", s)
		}
	}
}

func TestBundleSummarize_NoChanges(t *testing.T) {
	// Create a bundle with no drift or events
	tmpDir := t.TempDir()
	bundleDir := filepath.Join(tmpDir, "empty-bundle")
	...
	// Verify no changes
	if summary.Changes.DriftCount != 0 {
		t.Errorf("DriftCount = %d, want 0", summary.Changes.DriftCount)
	}
	if summary.Changes.EventCount != 0 {
		t.Errorf("EventCount = %d, want 0", summary.Changes.EventCount)
	}
	if len(summary.RiskSignals) != 0 {
		t.Errorf("RiskSignals = %d, want 0", len(summary.RiskSignals))
	}

	// Verify ASCII output shows "No drift or events detected"
	output := renderSummaryASCII(summary)
	if !strings.Contains(output, "No drift or events detected") {
		t.Error("ASCII output should indicate no changes")
	}
}

func TestBundleSummarize_RiskSignals(t *testing.T) {
	// Create a bundle with critical and warning findings
	tmpDir := t.TempDir()
	bundleDir := filepath.Join(tmpDir, "risk-bundle")
	...
	bundle := &agent.DebugBundle{
		...
		DriftFindings: []agent.DriftFinding{
			{ID: "d1", ObjectID: "Deployment:prod/api", Severity: agent.DriftSeverityCritical},
			{ID: "d2", ObjectID: "Deployment:prod/api", Severity: agent.DriftSeverityWarning},
			{ID: "d3", ObjectID: "Deployment:prod/api", Severity: agent.DriftSeverityWarning},
		},
		Events: []agent.TimelineEvent{
			{ResourceKind: "Pod", K8sEvent: agent.K8sEvent{Type: "Warning"}},
		},
	}
	...
	// Verify risk signals
	if len(summary.RiskSignals) != 3 {
		t.Errorf("RiskSignals = %d, want 3 (1 critical, 1 warning drift, 1 warning event)", len(summary.RiskSignals))
	}

	// Check for critical signal
	hasCritical := false
	for _, r := range summary.RiskSignals {
		if r.Level == "critical" {
			hasCritical = true
			break
		}
	...
	if !hasCritical {
		t.Error("Expected critical risk signal")
	}
}
...
func TestBundleSummarize_Deterministic(t *testing.T) {
	// Create a bundle
	tmpDir := t.TempDir()
	bundleDir := filepath.Join(tmpDir, "determinism-bundle")
	...
	bundle := &agent.DebugBundle{
		...
		DriftFindings: []agent.DriftFinding{
			{ID: "d3", ObjectID: "Deployment:prod/c", Classification: agent.DriftConfig},
			{ID: "d1", ObjectID: "Deployment:prod/a", Classification: agent.DriftImage},
			{ID: "d2", ObjectID: "Deployment:prod/b", Classification: agent.DriftCapacity},
		},
	}
	...
	// Verify categories and objects are sorted
	summary := buildBundleSummary(&agent.DebugBundle{
		DriftFindings: bundle.DriftFindings,
	}, bundleDir)

	// Categories should be sorted
	for i := 1; i < len(summary.Changes.Categories); i++ {
		if summary.Changes.Categories[i] < summary.Changes.Categories[i-1] {
			t.Errorf("Categories not sorted: %v", summary.Changes.Categories)
			break
		}
	}

	// Objects should be sorted
	for i := 1; i < len(summary.Changes.Objects); i++ {
		if summary.Changes.Objects[i] < summary.Changes.Objects[i-1] {
			t.Errorf("Objects not sorted: %v", summary.Changes.Objects)
			break
		}
	}
}
